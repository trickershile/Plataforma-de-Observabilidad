...
header_main = '<div class="main-header">Panel de Control Global - POIA-IoT</div>'
header_sub = '<div class="sub-header">Plataforma de Observabilidad con IA e Infraestructura DataOps</div>'
st.markdown(header_main, unsafe_allow_html=True)
st.markdown(header_sub, unsafe_allow_html=True)
...
def _crear_grafico_tendencia(df, var_label, var_col):
    unidad = VAR_UNITS.get(var_col, "")
    media = df[var_col].mean()
    desviacion = df[var_col].std()
    umbral_superior = media + 2 * desviacion
    umbral_inferior = media - 2 * desviacion

    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=df['timestamp'], y=df[var_col],
        mode='lines', name=var_label,
        line=dict(color="#7BA7C9", width=2),
        hovertemplate=(
            "<b>%{x|%Y-%m-%d %H:%M:%S}</b><br>%{y:.2f} " + unidad +
            "<br>Sensor: %{customdata}<extra></extra>"
        ),
        customdata=df['sensor_id'],
    ))
    ...
    if var_col in ('temperatura_limpia', 'vibracion_limpia'):
        ...
        if ma_col in df.columns:
            fig.add_trace(go.Scatter(
                x=df['timestamp'], y=df[ma_col],
                mode='lines', name='Media Movil',
    ...
    fig.add_trace(go.Scatter(
        ...
        y=pd.concat([
            pd.Series([umbral_superior] * len(df)),
            pd.Series([umbral_inferior] * len(df))
        ]),
        fill='toself', fillcolor='rgba(183, 164, 214, 0.15)',
        line=dict(color='rgba(183, 164, 214, 0)'),
        name='Rango Normal (+/- 2 sigma)', showlegend=True,
        hovertemplate=(
            "Umbral superior: %.2f %s<br>Umbral inferior: %.2f %s<extra></extra>"
            % (umbral_superior, unidad, umbral_inferior, unidad)
        ),
    ))

    puntos_anomalos = df[df['is_anomaly'] == 1]
    if not puntos_anomalos.empty:
        fig.add_trace(go.Scatter(
            x=puntos_anomalos['timestamp'], y=puntos_anomalos[var_col],
            mode='markers', name='Anomalia',
            marker=dict(color='#D17A7A', size=9, symbol='circle',
                        line=dict(color='#B05A5A', width=1.5)),
            hovertemplate=(
                "<b>%{x|%Y-%m-%d %H:%M:%S}</b><br>Valor: %{y:.2f} " + unidad +
                "<br>Estado: " + puntos_anomalos['estado_alerta'] + "<extra></extra>"
            ),
        ))

    anomalias_count = df['is_anomaly'].sum()
    if anomalias_count > 0:
        pct_anomalo = len(puntos_anomalos) / len(df) * 100
        kpi_text = f"Detectadas {anomalias_count} anomalias ({pct_anomalo:.1f}% de los datos)"
    else:
        kpi_text = "Sin anomalias detectadas en el periodo"

    fig.update_layout(
        title=dict(text=f"{var_label} - {kpi_text}", font=dict(color="#4A6FA5", size=16), x=0, xanchor='left'),
        plot_bgcolor="#F5F7FA", paper_bgcolor="#FFFFFF",
        font=dict(color="#4A5568", size=12),
        margin=dict(l=40, r=20, t=60, b=40),
        hovermode="x unified",
        xaxis=dict(showgrid=True, gridcolor="#EDF2F7", title="Tiempo",
                   title_font=dict(color="#6B7B8D", size=13),
                   rangeslider=dict(visible=True, thickness=0.08), type="date"),
        yaxis=dict(showgrid=True, gridcolor="#EDF2F7",
                   title=f"{var_label} ({unidad})" if unidad else var_label,
                   title_font=dict(color="#6B7B8D", size=13)),
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1, font=dict(size=11)),
        hoverlabel=dict(bgcolor="#FFFFFF", font_size=12, font_color="#4A5568"),
    )
    return fig


def _mostrar_tabla_auditoria(df, anomalias_detectadas):
    ...
    sc1.markdown(
        f'<div class="metric-card green"><div class="metric-label">Registros Normales</div>'
        f'<div class="metric-value" style="font-size:1.4rem">{total_ok:,}</div></div>',
        unsafe_allow_html=True
    )
    sc2.markdown(
        f'<div class="metric-card pink"><div class="metric-label">Anomalias</div>'
        f'<div class="metric-value" style="font-size:1.4rem">{anomalias_detectadas}</div></div>',
        unsafe_allow_html=True
    )
    sc3.markdown(
        f'<div class="metric-card"><div class="metric-label">Sensores</div>'
        f'<div class="metric-value" style="font-size:1.4rem">{df["sensor_id"].nunique()}</div></div>',
        unsafe_allow_html=True
    )
    sc4.markdown(
        f'<div class="metric-card"><div class="metric-label">Periodo</div>'
        f'<div class="metric-value" style="font-size:0.9rem">{inicio_periodo}</div>'
        f'<div class="metric-delta">a {fin_periodo}</div></div>',
        unsafe_allow_html=True
    )

    col_tab_filtros = st.columns([1.3, 1, 1.2, 1.5])
    with col_tab_filtros[0]:
        modo_vista = st.radio("Filtro:", ["Todos", "Solo anomalias"], horizontal=True)
    ...
    columnas_mostrar = {
        'timestamp': 'Timestamp', 'sensor_id': 'Sensor',
        'temperatura_limpia': 'Temperatura (C)', 'vibracion_limpia': 'Vibracion (g)',
        'voltaje': 'Voltaje (V)', 'temp_promedio_movil': 'Temp. Media Movil',
        'vib_desviacion_movil': 'Vib. Desviacion', 'estado_alerta': 'Estado',
    }
...
with tab1:
    section_title = '<div class="section-title">Estado Operativo de la Planta en Tiempo Real</div>'
    ...
    if os.path.exists(RUTA_GOLD):
        ...
        _mostrar_metricas(df, total_mediciones, anomalias_detectadas, porcentaje_falla)

        st.markdown('<hr class="divider">', unsafe_allow_html=True)

        section_trend = '<div class="section-title">Analisis de Tendencia y Aislamiento de Fallas</div>'
        st.markdown(section_trend, unsafe_allow_html=True)
        ...
        fig = _crear_grafico_tendencia(df_filtrado, var_label, var_col)
        st.plotly_chart(fig, width='stretch')

        st.markdown('<hr class="divider">', unsafe_allow_html=True)
        st.markdown('<div class="section-title">Registros de Auditoria</div>', unsafe_allow_html=True)

        _mostrar_tabla_auditoria(df, anomalias_detectadas)
...
with tab2:
    ...
    st.markdown('<hr class="divider">', unsafe_allow_html=True)
    section_log = '<div class="section-title">Bitacora de Errores de Calidad y Esquema (DER)</div>'
    st.markdown(section_log, unsafe_allow_html=True)
